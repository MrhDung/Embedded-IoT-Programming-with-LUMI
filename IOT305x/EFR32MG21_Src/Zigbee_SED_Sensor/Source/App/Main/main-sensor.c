...
void mainStateEventHandler (void)
{
	...
	switch(g_systemState)
	{
		...
		case LEAVE_NETWORK:
		{
			SEND_TempValueReport(ENDPOINT_1, 0);
			SEND_HumiValueReport(ENDPOINT_1, 0);
			SEND_LDRValueReport(ENDPOINT_2, 0);
			SEND_LD2410StateReport(ENDPOINT_3, 0);
	...
	if(button == SW1)
	{
		switch(pressCount)
		{
			...
			case press_3:
			{
				...
				if (bindingEnable == 1)
				{
					emberAfPluginFindAndBindInitiatorStart(ENDPOINT_3);
				}
				else
				{
					emberClearBindingTable();
