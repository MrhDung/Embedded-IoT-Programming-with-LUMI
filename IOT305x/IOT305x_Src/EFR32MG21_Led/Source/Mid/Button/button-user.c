...
static Button_t buttonArr[BUTTON_NUMBER] = BUTTON_INIT;
...
void emberAfHalButtonIsrCallback (uint8_t button, uint8_t state)
{
	...
	else if (state == BUTTON_RELEASED)
	{
		...
		if(Timer_CalculatorTime(buttonArr[g_index].timePress, currentTime) <= 50)
		{
			buttonArr[g_index].countButton--;
		}
		else
...
void scanButtonEventHandler (void)
{
	...
	for (uint8_t i = 0; i < BUTTON_NUMBER; i++)
	{
		...
		if (buttonArr[i].countButton != 0)
		{
			...
			{
				if (Timer_CalculatorTime(buttonArr[i].timePress, TimeCurrent) == 1000)
				{
					g_holdCallback(buttonArr[i].pin, hold_1s);
					buttonArr[i].holdFlag = 1;
				}
				else if (Timer_CalculatorTime(buttonArr[i].timePress, TimeCurrent) == 2000)
				{
					g_holdCallback(buttonArr[i].pin, hold_2s);
					buttonArr[i].holdFlag = 1;
				}
				else if (Timer_CalculatorTime(buttonArr[i].timePress, TimeCurrent) == 3000)
				{
			...
			else
			{
				...
				{
					if (Timer_CalculatorTime(buttonArr[i].timePress, TimeCurrent) >= 400)
					{
